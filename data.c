...
#else
static int f2fs_write_cache_pages_inode(struct address_space *mapping,
					struct writeback_control *wbc,
					enum iostat_type io_type);
static int f2fs_write_cache_pages(struct address_space *mapping,
					struct writeback_control *wbc,
					enum iostat_type io_type)
{
	...
	if (get_dirty_pages(mapping->host) <=
		...
		clear_inode_flag(mapping->host, FI_HOT_DATA);
 	if (S_ISDIR(mapping->host->i_mode)) {//目录文件
		printk("DIR is writing");
 		return f2fs_write_cache_pages_inode(mapping,wbc,io_type);
	}
	if (wbc->range_cyclic) {
		writeback_index = mapping->writeback_index; /* prev offset */
		index = writeback_index;
		...
		if (wbc->range_start == 0 && wbc->range_end == LLONG_MAX)//shell 中运行sync
			range_whole = 1;
		cycled = 1; /* ignore range_cyclic tests */
	}
	//if(wbc->range_end==ULONG_MAX) return 0;//BUG来源
	tag = PAGECACHE_TAG_TOWRITE;
	/*if (wbc->sync_mode == WB_SYNC_ALL || wbc->tagged_writepages)
		tag = PAGECACHE_TAG_TOWRITE;
	else
...
continue_unlock:
			...
			if (unlikely(ret)) {
				...
				done_index = page->index + 1;//必须要有,记录完成的最后一个页索引
				done = 1;
				break;
			} else if (submitted) {
			 	last_idx = page->index;
...
continue_unlock:
			...
			ret = __write_data_page(page, &submitted, wbc, io_type);
			printk("__write_data_page： inode=%d index=%ld\n",page->mapping->host->i_ino, page->index);//会按照index从小到大的次序依次下刷。
			if (unlikely(ret)) {
				/*
				 * keep nr_to_write, since vfs uses this to
				 * get # of written pages.
